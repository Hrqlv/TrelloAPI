...
test("criar coluna done", async ({ request }) => {
  const response = await request.post(`${BASE_URL}/lists`, {
    headers: {
      Authorization: AUTH,
    },
  ...
  const body = JSON.parse(await response.text());
  idDone = body.id;
  console.log("id da coluna in done", idDone);
});
...
test("criar card 1", async ({ request }) => {
  ...
  //console.log(body)
  idBacklog= body.id;
  console.log("id da coluna in backlog", idBacklog);
});
...
test("criar card 3", async ({ request }) => {
  const response = await request.post(`${BASE_URL}/cards`, {
    ...
    params: { name: "card 3", idList: idReview, desc: "descricao" },
  });
  expect(response.ok()).toBeTruthy();
  const body = JSON.parse(await response.text());
  console.log(body);
...
test("criar card 4", async ({ request }) => {
  const response = await request.post(`${BASE_URL}/cards`, {
    headers: {
      Authorization: AUTH,
    },
  ...
  expect(response.ok().toBeTruthy());
  id = body.id;
  console.log("id da coluna done", idDone);
})